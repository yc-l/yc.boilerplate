...
namespace YC.DapperFrameWork.LambdaExtensions
{
    ...
    public class DapperExtensionsConfiguration : IDapperExtensionsConfiguration
    {
        ...
        public IClassMapper GetMap(Type entityType)
        {
            ...
            if (!_classMaps.TryGetValue(entityType, out map))
            {
                ...
                //过滤  NotMapped 标识的字段
                map.Properties = map.Properties.Where(x => !x.MemberInfo.CustomAttributes.Any(t => t.AttributeType == typeof(NotMappedAttribute))).ToList();

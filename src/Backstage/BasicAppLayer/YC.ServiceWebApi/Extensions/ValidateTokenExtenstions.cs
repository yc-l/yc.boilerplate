        ...
        public static void  ValidateToken(string token, IHttpContextAccessor httpContextAccessor, ICacheManager cacheManager)
        {
            ...
            var tokenValidate = TokenContext.Validate(token, (load) =>
            {
                ...
                try
                {
                    ...
                    if (userInfo == null)
                        throw new Exception(DefaultConfig.DefaultAppConfigDto.ExceptionKey + "Token 无效，请从新获取Token！");
                    if (userInfo.Token != token)
                        throw new Exception(DefaultConfig.DefaultAppConfigDto.ExceptionKey + "Token 信息不一致，请重新获取token！");

                    if (userInfo.IP != IPUtils.GetIP(httpContextAccessor?.HttpContext?.Request)) {
                        throw new Exception(DefaultConfig.DefaultAppConfigDto.ExceptionKey + "Token 留存IP信息不一致！");
                    }
                    return true;
                }
                catch (Exception ex)
                {
                    if (ex.Message.Contains(DefaultConfig.DefaultAppConfigDto.ExceptionKey))
                    {
                        throw ex;
                    }
                    else {
                        throw new Exception(DefaultConfig.DefaultAppConfigDto.ExceptionKey + "获取租户信息失败！");
                    }
            ...
            if (!tokenValidate)
            {
                throw new Exception(DefaultConfig.DefaultAppConfigDto.ExceptionKey + "Token 验证失败，请从新获取Token！");
            }
        }
    }
}
